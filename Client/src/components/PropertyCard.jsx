...
const PropertyCard = ({ property, viewMode = 'grid', onToggleFavorite, onToggleComparison, isFavorite = false, isInComparison = false }) => {
  ...
  const handleComparison = async (e) => {
    ...
    if (onToggleComparison) {
      await onToggleComparison(property._id);
    }
  };
  ...
  return (
    <Link to={`/property/${property._id}`} className="block">
      <AnimatedCard
        ...
        className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group w-full h-[320px] flex flex-col cursor-pointer"
        hover={true}
      >
      ...
      <div className="p-4 flex-grow flex flex-col justify-between">
        <div className="flex-grow">
          <div className="flex flex-col justify-between items-start mb-2 gap-1">
            <h3 className="text-lg font-bold text-gray-900 line-clamp-2 min-h-[3.5rem]">{property.title}</h3>
            <span className="text-xl font-bold text-blue-600">{formatPrice(property.price)}</span>
          </div>
          
          <div className="flex items-center text-gray-600 mb-3">
            <MapPin className="w-4 h-4 mr-1 flex-shrink-0" />
            <span className="text-sm truncate">{property.location.city}, {property.location.region}</span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            <div className="flex items-center text-gray-600">
              <Square className="w-4 h-4 mr-1 flex-shrink-0" />
              <span className="text-sm">{property.squareFootage ? `${property.squareFootage} sq ft` : 'N/A'}</span>
            </div>
          </div>
        </div>
